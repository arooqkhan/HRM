    ...
    public function index()
{
    ...
    if ($user->role === 'admin' || $user->role === 'HR' || $user->role === 'Accountant') {
        // Show all records if the user is an admin, HR, or Accountant
        $accouncementdocuments = AccouncementDocument::with('employee')->get();
    } else {
        // Show only the records that belong to the currently authenticated user
    ...
    return view('admin.pages.accouncementdocument.index', compact('accouncementdocuments'));
}
...
{
    $document = AccouncementDocument::findOrFail($id);
    $document->status = 1; // Change status to 'Uploaded'
    $document->save();
